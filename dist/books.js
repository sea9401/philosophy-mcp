...
function isBlockedIp(ip) {
    const v = isIP(ip);
    if (v === 4)
        return isBlockedIp4(ip);
    if (v === 6)
        return isBlockedIp6(ip);
...
async function assertPublicUrl(raw) {
    ...
    if (!host)
        throw new Error(`URL has no host: ${raw}`);
 ...
 */
async function bookRequest(url, opts = {}) {
...
const GUTENDEX_OPTS = { retries: 0, timeoutMs: 8_000 };
 ...
 */
const bodyCache = new LruCache(8);
...
const offsetField = z
    ...
    .min(0)
    .default(0)
    .describe("Character offset to start from (default 0). For long texts, read a window then call " +
...
async function gutenbergOpds(terms, limit) {
    ...
    for (const entry of asArr(root?.feed?.entry)) {
        ...
        if (bid === null)
            continue; // skip facet/navigation entries
...
export function registerBookTools(server) {
    // --- Project Gutenberg ---------------------------------------------------
    server.registerTool("search_gutenberg", {
        title: "Search Project Gutenberg (public-domain classics)",
        description: "Search Project Gutenberg for public-domain classics and out-of-copyright translations " +
        ...
        try {
            ...
            if (languages)
                params.set("languages", languages.replace(/\s/g, ""));
            ...
            if (results.length) {
                const lines = results.map((b) => {
                    ...
                    const authors = asArr(b.authors).map((a) => a.name).join(", ") || "—";
                    return [
                        `• [${b.id}] ${b.title ?? "?"}`,
                        `   author: ${authors} | lang: ${asArr(b.languages).join(",")} | downloads: ${b.download_count ?? 0}`,
        ...
        if (!queryTerms)
            return fail("Provide a query, author, or topic to search Project Gutenberg.");
        try {
            const items = await gutenbergOpds(queryTerms, limit);
            if (!items.length)
                return ok("No Project Gutenberg matches.");
    ...
    server.registerTool("get_gutenberg_text", {
        title: "Read a Project Gutenberg book",
        description: "Fetch the plain-text body of a Project Gutenberg book by numeric id (from search_gutenberg). " +
        ...
        if (!body) {
            ...
            if (!body.trim()) {
                ...
                try {
                    ...
                    if (url)
                        body = await getText(url, { retries: 1 });
                }
                catch {
                    /* ignore */
                }
            ...
            if (!body.trim()) {
                return fail(`Could not download plain text for Gutenberg id ${bid}. ` +
    ...
    // --- Internet Archive ----------------------------------------------------
    server.registerTool("search_internet_archive", {
        title: "Search the Internet Archive",
        description: "Search the Internet Archive for scanned, out-of-print monographs, journals, and " +
        ...
        try {
            ...
            if (!docs.length)
                return ok("No Internet Archive matches.");
    ...
    server.registerTool("get_archive_text", {
        title: "Read Internet Archive OCR text",
        description: "Fetch the OCR full text of an Internet Archive item by identifier (from " +
        ...
        if (!body) {
            ...
            try {
                ...
                for (const f of asArr(meta.files)) {
            ...
            if (!body.trim()) {
                return fail(`No OCR text file found for "${identifier}" (may be image-only or restricted). ` +
    ...
    // --- Wikisource ----------------------------------------------------------
    server.registerTool("search_wikisource", {
        title: "Search Wikisource",
        description: "Search Wikisource for primary texts and translations (en, de, ko, ...). " +
        ...
        try {
            ...
            if (!hits.length)
                return ok(`No ${lang}.wikisource matches.`);
    ...
    server.registerTool("get_wikisource_text", {
        title: "Read a Wikisource page",
        description: "Fetch the plain text of a Wikisource page by title (from search_wikisource). Returns one " +
    ...
    // --- Open Library --------------------------------------------------------
    server.registerTool("search_openlibrary", {
        title: "Search Open Library",
        description: "Search Open Library for books — strong for modern editions and translations as metadata, " +
        ...
        try {
            ...
            if (!docs.length)
                return ok("No Open Library matches.");
            const lines = docs.map((d) => {
                const authors = asArr(d.author_name).join(", ") || "—";
                const ia = asArr(d.ia);
    ...
    // --- DOAB ----------------------------------------------------------------
    server.registerTool("search_doab", {
        title: "Search DOAB (open-access books)",
        description: "Search the Directory of Open Access Books — peer-reviewed, fully open-access academic " +
        ...
        try {
            ...
            if (!Array.isArray(data) || !data.length)
                return ok(`No DOAB matches. Browse: ${manual}`);
            const lines = data.slice(0, limit).map((item) => {
                const mds = asArr(item.metadata).filter((m) => m && typeof m === "object");
                const vals = (...keys) => mds.filter((m) => keys.includes(m.key) && m.value).map((m) => String(m.value));
    ...
    server.registerTool("search_sep", {
        title: "Search the Stanford Encyclopedia of Philosophy",
        description: "Search SEP (the standard scholarly reference) by matching its full entry index. " +
        ...
        try {
            ...
            if (!words.length)
                return fail("Provide a search term.");
            let matches = idx.filter(([t]) => words.every((w) => t.toLowerCase().includes(w)));
            if (!matches.length)
                matches = idx.filter(([t]) => words.some((w) => t.toLowerCase().includes(w)));
            if (!matches.length)
                return ok(`No SEP entries matched "${query}". Browse https://plato.stanford.edu/contents.html`);
    ...
    server.registerTool("get_sep_entry", {
        title: "Read a Stanford Encyclopedia entry",
        description: "Fetch the text of a SEP entry. Accepts a slug (e.g. 'hegel'), a /entries/.. path, or a " +
    ...
    // --- Catch-all -----------------------------------------------------------
    server.registerTool("fetch_text", {
        title: "Fetch readable text from any URL",
        description: "Fetch any URL and return its readable text — the catch-all for sources without a " +
        ...
        try {
            ...
            if (text === undefined) {
                ...
                if (!text.trim())
                    return ok(`Fetched ${url} but found no readable text (binary or JS-only page?).`);
                bodyCache.set(key, text);
            }
