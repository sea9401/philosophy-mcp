 ...
 */
async function resolveAllowed(input) {
    if (!input || !input.trim())
    ...
    if (!allowed) {
        throw new Error(`Path "${real}" is outside the allowed roots (${roots.join(", ")}). ` +
...
async function loadDoc(real) {
    ...
    if (ext === ".pdf") {
        ...
        throw new Error(`Unsupported file type "${ext || "(none)"}" — this tool reads .pdf and plain-text files.`);
    }
    CACHE.set(real, doc);
    return doc;
}
...
function looksLikeHeading(line) {
    ...
    if (t.length < 2 || t.length > 90)
        return false;
    if (/[.,;:]$/.test(t))
        return false; // sentence-like → not a heading
    if (HEADING_RES.some((re) => re.test(t)))
        return true;
...
export function registerLocalTools(server) {
    server.registerTool("local_doc_info", {
        title: "Local document overview",
        description: "Open a local PDF or plain-text file and return a cheap overview — page count, " +
    ...
    server.registerTool("local_doc_search", {
        title: "Search inside a local document",
        description: "Search a local PDF or text file for a keyword or regular expression and return only the " +
        ...
        try {
            ...
            outer: for (let p = 0; p < doc.pages.length; p++) {
                ...
                while ((m = re.exec(text)) !== null) {
                    ...
                    if (++perPage >= 50)
                        break; // don't let one page dominate
            ...
            const more = truncatedCollection || hits.length > shown.length || cutByChars
                ? " (more matches exist — refine the query or read the cited pages)"
                : "";
            const header = `${hits.length}${truncatedCollection ? "+" : ""} match(es) for ${is_regex ? `/${query}/` : `"${query}"`} in ${basename(real)}; showing ${shown.length}${more}.`;
    ...
    server.registerTool("local_doc_read", {
        title: "Read a page range of a local document",
        description: "Read a bounded slice of a local PDF or text file — a page range starting at page_from — and " +
        ...
        try {
            ...
            const next = lastRead < total
                ? ` Continue with page_from=${lastRead + 1}.`
                : " (end of document).";
            const header = `${basename(real)} — pages ${start}–${lastRead} of ${total}` +
